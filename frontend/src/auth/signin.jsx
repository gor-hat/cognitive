...
import {Password, Username ,RememberPassword,ForgotPassword ,JWT,LoginWithJWT } from '../constant'; // SignIn,FIREBASE,AUTH0, CreateAccount

const Logins = (props) => {
  
    //const {loginWithRedirect} = useAuth0()
    const [usuario, setUsuario] = useState("matricula");
    ...
    const loginWithJwt = (usuario,password) => {
                ...
                <TabContent activeTab={selected} className="content-login">
                  <TabPane  className="fade show" tabId={"jwt"}>
                    <Form className="theme-form">
                      <h4>{"Bem vindo!"}</h4>
                      <p>{"Insira seu usuario e senha para entrar"}</p>
                      <FormGroup>
                        <Label className="col-form-label">{Username}</Label>
                        <Input className="form-control" type="email" required="" onChange={e => setUsuario(e.target.value)} defaultValue={usuario} />
                      </FormGroup>
                      <FormGroup>
                      ...
                      <div className="form-group mb-0">
                        ...
                        :*/}
                        <Button color="primary" className="btn-block" onClick={() => loginWithJwt(usuario,password)}>{LoginWithJWT}</Button>
